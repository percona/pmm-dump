...
func TestEncryptionExportImport(t *testing.T) {
	...
	pmm.Log("Exporting data to check keys", filepath.Join(testDir, "dump-just-key.tar.gz"))
	args = []string{"-d", filepath.Join(testDir, "dump-just-key.tar.gz"), "--pmm-url", pmm.PMMURL(), "--dump-qan", "--click-house-url", pmm.ClickhouseURL(), "--pass", "somepass", "--just-key"}
	stdout, stderr, err = b.Run(append([]string{"export", "--ignore-load"}, args...)...)
	if err != nil {
		t.Fatal("failed to export", err, stdout, stderr)
	}

	want := `Password: somepass`
